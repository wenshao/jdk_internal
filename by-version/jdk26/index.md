...
| **Scoped Values（正式版）** | ⭐⭐⭐⭐⭐ | 正式版 |
| **结构化并发** | ⭐⭐⭐⭐ | 第六次预览 |
| **原始类型模式匹配** | ⭐⭐⭐⭐ | 第四次预览 |
| **延迟常量** | ⭐⭐⭐ | 第二次预览 |
| **Vector API** | ⭐⭐⭐ | 第十一次孵化 |
| **分代 Shenandoah（正式版）** | ⭐⭐⭐⭐ | 正式版 |
| **严格 final 语义** | ⭐⭐⭐ | 准备未来版本 |
...
| **Scoped Values (Final)** | 正式版 | 作用域值 |

### 性能

| JEP | 标题 | 影响 | 描述 |
|-----|------|------|------|
| [JEP 522](https://openjdk.org/jeps/522) | G1 GC Throughput | 🚀 +10-20% | 减少同步提升吞吐量 |
...
| 领域 | 改进 | 数据 |
|------|------|------|
| **G1 GC** | 吞吐量提升 | +10-20% (减少同步) |
| **启动时间** | AOT 对象缓存 | 显著提升 |
| **HTTP/3** | 连接建立 | 0-RTT 恢复 (vs TCP 2-3 RTT) |
...
- **HTTP/3** (JEP 517)：基于 QUIC 协议，支持 0-RTT 连接恢复

### GC

- **G1 吞吐量提升** (JEP 522)：减少同步开销，提升 10-20% 吞吐量
