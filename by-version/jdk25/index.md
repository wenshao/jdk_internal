...
| **Compact Object Headers** | ⭐⭐⭐⭐⭐ | 内存优化，减少对象头开销 |
...
| **Module Import Declarations** | ⭐⭐⭐⭐ | 预览，简化模块导入 |
| **原始类型模式匹配** | ⭐⭐⭐⭐ | 第三次预览 |
| **Stable Values** | ⭐⭐⭐⭐ | 预览，稳定值 |
| **Compact Source Files** | ⭐⭐⭐ | 简化源文件 |
| **Vector API** | ⭐⭐⭐ | 第十次孵化 |
...
| JEP 511 | Module Import Declarations | 🔍 预览 | 模块导入声明 | → |
| JEP 502 | Stable Values | 🔍 预览 | 稳定值 | → |
| JEP 512 | Compact Source Files and Instance Main Methods | 🔍 预览 | 简化源文件 | → |
...
| JEP | 标题 | 影响 | 描述 | 深度分析 |
|-----|------|------|------|----------|
| [JEP 519](/jeps/gc/jep-519.md) | Compact Object Headers | 🚀 内存优化 | 紧凑对象头 | → |
...
- **Compact Object Headers** (JEP 519)：减少对象头开销
...
| String Templates 撤销 | 使用预览 String Templates | 等待重新设计或使用传统方式 |
...
| 场景 | 推荐特性 | 深度分析 |
|------|----------|----------|
| 替代 ThreadLocal | Scoped Values | [→](/deep-dive/jep-506-implementation.md) |
| 大内存应用 | ZGC (默认分代模式) | → |
| 高并发 I/O | 虚拟线程 | → |
| 并发任务 | 结构化并发 | → |
