# Leslie Zhai (xiangzhai)

> **Loongson** | **Beijing** | **RISC-V/LoongArch**

---

## 基本信息

| 属性 | 值 |
|------|-----|
| **GitHub** | [@xiangzhai](https://github.com/xiangzhai) |
| **姓名** | Leslie Zhai |
| **位置** | Beijing, China |
| **组织** | [Loongson](../../contributors/orgs/loongson.md) |
| **主要领域** | RISC-V, LoongArch |
| **Bio** | Good Morning Cat |

---

## 社交网络关联

| 关联类型 | 用户 | 推断 |
|----------|------|------|
| **龙芯联系** | kuaiwei | Kuai Wei, Alibaba, C2 编译器 |
| **社区联系** | sanhong | Sanhong Li, Alibaba JVM Team |
| **社区联系** | luchsh | Jonathan Lu, 上海 |

**地理位置**: 北京
**组织**: 龙芯 (Loongson)

---

## 技术方向

基于社交网络和 GitHub 活动推断：
- **RISC-V/LoongArch**: 龙芯架构支持
- **JVM 移植**: 与阿里巴巴 JVM 团队有联系，可能参与龙芯 JVM 移植工作

---

## 外部链接

| 类型 | 链接 |
|------|------|
| **GitHub** | https://github.com/xiangzhai |

---

## 相关文档

- [Kuai Wei](kuai-wei.md) - 阿里巴巴 C2 编译器专家（RISC-V 贡献）
- [中国贡献者](chinese-contributors.md) - 中国 OpenJDK 贡献者

---

**最后更新**: 2026-03-21
