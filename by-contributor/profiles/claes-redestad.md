...
Claes Redestad (OpenJDK 用户名：**redestad**) 是 Oracle 的资深 JDK 开发者，Java SE Performance 团队核心成员，专注于启动时间优化、字符串拼接和字符集编码/解码等性能优化工作。2011 年底加入 Oracle，2022 年 3 月由 Daniel Fuchs 提名成为 OpenJDK Member Group 成员。

他是 **JEP 254: Compact Strings** (JDK 9) 的作者，该优化将 String 内部表示从 UTF-16 `char[]` 改为 `byte[]` 加编码标识，显著减少了内存占用。
...
> - 更新外部资源章节，移除不可访问的 Inside.java 链接
> - 添加本地 JEP 文档链接
> - 改进章节编号和格式
