...
> **数据来源**: [gotopia.tech](https://gotopia.tech/experts/839/paul-sandoz), [The Org](https://theorg.com/org/oracle/org-chart/paul-sandoz), [Inside.java](https://inside.java/u/PaulSandoz/), [LinkedIn](https://www.linkedin.com/in/paul-sandoz-4704562/)
...
Paul Sandoz 是 Stream API 的核心实现者之一，与 Brian Goetz 共同将函数式编程范式引入 Java。

```java
// Stream API - Paul Sandoz 参与设计和实现的核心 API
...
**核心设计目标**:
- 提供可移植的跨架构向量计算 API
- 在支持的 CPU 架构上编译为高效的 SIMD 指令 (SSE, AVX, NEON)
- 适用于机器学习、线性代数、密码学、金融计算等领域
- 优雅降级到标量操作

### 3. VarHandle (JEP 193, JDK 9)

VarHandle 提供了对变量的类型安全、高性能访问机制：

```java
// VarHandle - 替代 sun.misc.Unsafe 的标准 API
...
vh.setVolatile(obj, 42);            // volatile
vh.compareAndSet(obj, 42, 43);      // CAS
```

### 4. Code Reflection

Paul Sandoz 近年来参与了 **Code Reflection** 的开发，该技术为 Java 提供代码模型的反射能力，支持在运行时访问和分析 Lambda 表达式和方法的结构化表示。这是 Java 支持 AI/ML 计算图和 GPU 编程的重要基础设施。

### 5. JAX-RS / Jersey (Sun Microsystems 时期)

在 Sun Microsystems 时期，Paul Sandoz 共同领导了 **JSR 311 (JAX-RS)** 规范的制定，并主导了 **Jersey** 框架的实现：
- JAX-RS: Java 平台的 RESTful Web Services 标准 API
- Jersey: JAX-RS 的参考实现，至今仍在广泛使用
...
Paul Sandoz 的独特之处在于他从云平台到 JVM 底层的全栈经历：

| 层次 | 工作 | 时期 |
|------|------|------|
| **云平台** | CloudBees | ~2010-2012 |
| **中间层** | JAX-RS/Jersey (Sun) | ~2005-2010 |
| **平台核心** | Stream API, VarHandle, Vector API (Oracle) | 2012-至今 |

### Vector API 的渐进式演进

Vector API 的 11 轮孵化体现了 OpenJDK 社区对 API 稳定性的高度重视：
- 等待 Project Valhalla 的 Value Types 以优化内存布局
- 随着 JDK 版本持续改进 API 表面
- 确保跨平台可移植性和性能一致性
...
| GOTO Amsterdam 2014 | GOTO Conference | Java 平台技术 |

### 播客

- **Inside Java Podcast Episode 7** (2020-11-17): "The Vector API" — 与 John Rose 讨论 Vector API 的设计与实现
...
- [Inside.java - PaulSandoz](https://inside.java/u/PaulSandoz/)
- [GitHub - PaulSandoz](https://github.com/PaulSandoz)
- [LinkedIn](https://www.linkedin.com/in/paul-sandoz-4704562/)
- [gotopia.tech Speaker Profile](https://gotopia.tech/experts/839/paul-sandoz)
- [The Org - Oracle](https://theorg.com/org/oracle/org-chart/paul-sandoz)
...
- [The Vector API in JDK 17 - Dev.Live](https://inside.java/2021/09/23/devlive-vector-api/)
- [Java for AI - Devoxx Belgium 2025 (PDF)](https://cr.openjdk.org/~psandoz/conferences/2025-Devoxx-Belgium/Devoxx-Belgium-25-Java-For-AI.pdf)
- [JVMLS 2023 - Code Reflection](https://inside.java/2023/08/28/code-reflection/)
...
- [Inside.java - PaulSandoz](https://inside.java/u/PaulSandoz/)
- [GitHub - PaulSandoz](https://github.com/PaulSandoz)
- [gotopia.tech - Paul Sandoz](https://gotopia.tech/experts/839/paul-sandoz)
- [JEP 338: Vector API](https://openjdk.org/jeps/338)
- [JEP 529: Vector API (Eleventh Incubator)](https://openjdk.org/jeps/529)
- [JEP 193: Variable Handles](https://openjdk.org/jeps/193)
- [Inside Java Podcast Ep. 7](https://inside.java/2020/11/17/podcast-007/)
- [LinkedIn - Paul Sandoz](https://www.linkedin.com/in/paul-sandoz-4704562/)
