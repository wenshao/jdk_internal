...
Hannes Wallnoefer (Wallnöfer) 是 Oracle 的资深软件工程师，是 OpenJDK javadoc 工具的核心维护者和最活跃的贡献者之一。他在 openjdk/jdk 仓库拥有约 550 次贡献，GitHub 上可追踪到 201 个已集成的 PR，其中绝大多数 (93%+) 与 javadoc 工具相关。早期他是 Nashorn JavaScript 引擎的共同开发者，与 Jim Laskey 和 A. Sundararajan 一起负责 Nashorn 的 AST、解析器和运行时实现。

在加入 Oracle 之前，Hannes 在开源社区中以 **Helma** 和 **RingoJS** 的创建者闻名。Helma 是一个基于 Mozilla Rhino 引擎的服务端 JavaScript 框架，RingoJS 则是其后继项目——一个构建在 JVM 上的 JavaScript 平台。他也是 Mozilla Rhino 项目的 Committer。他出生于意大利北部 (Meran)，后长期生活在维也纳，现居奥地利南部。

在 OpenJDK Census 中，他是 **Amber 项目 Committer**、**Detroit 项目 Committer**、JDK Reviewer/Committer 和 JDK Updates Reviewer。
...
| **当前组织** | Oracle |
| **位置** | 奥地利南部 (出生于意大利 Meran) |
| **GitHub** | [@hns](https://github.com/hns) |
| **OpenJDK** | [@hannesw](https://wiki.openjdk.org/display/~hannesw) |
| **专长** | Javadoc 工具、Nashorn JavaScript 引擎、API 文档生成 |
| **OpenJDK 角色** | Amber Committer, Detroit Committer, JDK Reviewer/Committer, JDK Updates Reviewer |
| **开源项目** | Helma (创建者), RingoJS (创建者), Mozilla Rhino (Committer) |
...
- [个人网站](https://hns.github.io/) - Hannes 的个人网站和技术博客
- [RingoJS](https://github.com/ringo/ringojs) - Hannes 创建的 JVM 上的 JavaScript 平台
- [Detroit Project](https://openjdk.org/projects/detroit/) - javax.script API 实现项目 (Committer)
