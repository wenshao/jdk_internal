...
| **职位** | JVM Developer (SAP JVM 团队成员, 2011年至今) |
| **位置** | 德国 |
...
| 年份 | 事件 | 详情 |
|------|------|------|
| **2011** | 加入 SAP | SAP JVM 团队，专注 HotSpot 后端开发 |
...
| 年份 | 活动 | 标题 |
|------|------|------|
| **2025** | FOSDEM (Free Java Devroom) | [Inner Workings of the FFI API in the JVM](https://archive.fosdem.org/2025/schedule/event/fosdem-2025-5647-inner-workings-of-the-ffi-api-in-the-jvm/) — 介绍 Foreign Function Interface 在 JVM 中的内部实现，聚焦 PowerPC 平台上的调用约定、栈布局和跨平台挑战 |

### 技术成就

- **FFI/Panama PowerPC 实现**：为 PowerPC 平台实现了 Foreign Function & Memory API (JEP 454) 的 JVM 底层支持
- **多 GC 平台移植**：为 PPC64 实现了 Shenandoah、ZGC 支持，以及 Virtual Threads 和 Foreign Function Linker
- **长期 PowerPC/AIX 维护**：作为 OpenJDK Reviewer 和被描述为 "true PowerPC/Itanium expert and memory model wizard"
...
| **OpenJDK Wiki** | [Martin Doerr](https://wiki.openjdk.org/display/~mdoerr) |
| **FOSDEM 2025** | [Speaker Page](https://archive.fosdem.org/2025/schedule/speaker/martin_doerr/) |
