...
| 平台 | 支持状态 | 主要架构 | 特有特性 |
|------|----------|----------|----------|
| **Linux** | 主要平台 | x64, aarch64, ppc64le, s390x, riscv64 | 容器优化、cgroup 支持 |
| **Windows** | 主要平台 | x64 (32 位已移除, JDK 24+) | 注册表集成、服务支持 |
| **macOS** | 主要平台 | x64, aarch64 (Apple Silicon) | Metal 渲染、公证要求 |
| **RISC-V** | Linux 平台 | riscv64 (RV64GV) | JDK 19 起主线集成，实验性 RVV/Zba/Zbb |
| **容器** | 跨平台 | 所有架构 | 资源限制感知、紧凑对象头优化 |
...
- JDK 11: 改进容器检测
- JDK 19: RISC-V 端口集成 (JEP 422, RV64GV)
- JDK 21: cgroup v2 完整支持
- JDK 24: 紧凑对象头实验性支持 (JEP 450)
- JDK 25: 紧凑对象头正式支持 (JEP 519)，内存减少 10-20%
...
- JDK 24: 移除 32 位 x86 端口
- JDK 25: 紧凑对象头支持 (JEP 519)
...
- JDK 11: 改进容器检测
- JDK 21: cgroup v2 支持
- JDK 25: 紧凑对象头 (JEP 519)，容器内存密度提升 10-20%
- JDK 26: AOT 类加载 (JEP 483)，容器冷启动显著改善
...
| **原生镜像** | ✅ | ✅ | ✅ | ✅ |
| **容器感知** | ✅ | ✅ | ✅ | ✅ |
| **紧凑对象头 (JDK 25+)** | ✅ x64/aarch64 | ✅ x64 | ✅ x64/aarch64 | ✅ |
...
JDK 17 ──── cgroup v2 改进
          │
JDK 19 ──── RISC-V 端口 (JEP 422)
          │
JDK 21 ──── cgroup v2 完整支持
          │
JDK 24 ──── 紧凑对象头实验 (JEP 450)
          │
JDK 25 ──── 紧凑对象头正式 (JEP 519)、JFR CPU 时间分析 (JEP 509)
          │
JDK 26 ──── AOT 类加载 (JEP 483)、容器启动优化
```
