...
紧凑对象头 (Project Lilliput)，对象头从 12→8 字节：
...
**内存节省**: 每个对象节省 4 字节; SPECjbb2015 堆空间减少 22%, CPU 时间减少 8%; 含大量小对象的应用活跃数据减少 10-20%

### JDK 26 性能改进

#### JEP 516: Ahead-of-Time Object Caching with Any GC

解除 AOT 缓存的 GC 限制，支持 ZGC 等所有垃圾收集器：

- **GC 无关缓存**: 对象引用存储为逻辑索引而非物理地址，后台线程逐个材料化对象
- **基线 AOT 缓存**: JDK 26 自带覆盖 JDK 类的基线 AOT 缓存，即使不进行训练运行也有基线启动改善
- **ZGC 支持**: JDK 24/25 的 AOT 特性不支持 ZGC，JDK 26 消除此限制

**性能影响**: 理想场景启动行为改善 30-70%
