...
| **2026.03** | JDK 26 | JEP 529 | Vector API 第十一孵化器；FFM API 持续优化 |
...
| 概念 | 说明 | 版本 |
|------|------|------|
| **ResourceScope** | 早期孵化器中的资源作用域 | JDK 17-18 |
| **MemorySession** | 替代 ResourceScope 的内存会话 | JDK 19 |
| **Arena** | 简化后的内存管理 API (MemorySession 拆分而来) | JDK 20+ (推荐) |
...
### Vector API (SIMD 向量化)
Vector API 是 Panama 项目的子项目，提供可移植的 SIMD 向量化计算能力。JDK 26 仍处于第十一轮孵化 (JEP 529)，等待 Project Valhalla 关键特性就绪后方可进入预览。
...
- [JEP 389: Foreign Linker API (Incubator)](/jeps/ffi/jep-389.md) - JDK 16 孵化器
- [JEP 383: Foreign-Memory Access API (Second Incubator)](https://openjdk.org/jeps/383) - JDK 15 孵化器
- [JEP 370: Foreign-Memory Access API (Incubator)](/jeps/ffi/jep-370.md) - JDK 14 孵化器
