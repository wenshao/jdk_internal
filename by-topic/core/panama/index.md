...
> **30 秒速读**
> - 目标彻底替代 JNI，纯 Java 调用原生函数，无需编写 C 胶水代码
> - FFM API JDK 22 正式发布 (JEP 454)，经 6 次孵化 + 3 次预览，历时 10+ 年
> - Arena 内存管理: 安全的堆外内存访问，自动释放，替代 `sun.misc.Unsafe`
> - jextract 工具: 从 C 头文件自动生成 Java 绑定代码
> - JEP 472 (JDK 24): 逐步限制 JNI 使用，推动迁移到 Panama
> - Vector API 仍在孵化中 (JDK 26 第十一孵化器, JEP 529)
...
JEP 370   JEP 383   JEP 393+389       JEP 412   JEP 419   JEP 424   JEP 434   JEP 442   JEP 454
孵化1     孵化2     孵化3+Linker孵化   孵化4     孵化5     预览1     预览2     预览3     ★ 正式
(内存)    (内存)    (内存+FFI)         (合并FFI) (合并)    (API重构) (精简)    (微调)    (GA)
```
...
| **性能** | ~50-100ns 调用开销 | ~10-20ns (JIT 可内联)（特定工作负载下，实际性能取决于调用模式和参数类型） |
...
**Panama** — 纯 Java，5 行（见 Section 5.1 示例）:

```java
// 完整代码见 Section 5.1 strlen 示例
// 核心: Linker.nativeLinker() + downcallHandle + Arena
```
...
> 以下数据为典型 x86-64 平台估算值，仅供参考（特定工作负载下，实际性能取决于调用模式和参数类型）。
...
> 以下为示意代码，实际 benchmark 需完整 JMH 配置。数据为典型 x86-64 平台估算值，仅供参考。
...
public class PanamaVsJniBenchmark {
    ...
    private MemorySegment testString;

    @Setup
    public void setup() {
        testString = Arena.global().allocateFrom("Hello!");
    }
...
// 典型估算结果 (JDK 22, x86-64):
// panamaStrlen  avgt  ~14  ns/op
// jniStrlen     avgt  ~73  ns/op   (~5x 慢)
```
...
- [JNI vs Panama 对比](/guides/comparisons/jni-vs-panama.md) — 从 JNI 到 Panama 的迁移动机与性能对比
- [Reflection 主题](../../language/reflection/) — Panama 替代反射访问的场景分析
- [JEP 454: Foreign Function & Memory API](/jeps/ffi/jep-454.md) — Panama 正式发布的里程碑 JEP
