...
| **JDK 21-22** | G1 / 分代 ZGC | 分代 ZGC 需 `-XX:+ZGenerational` 显式启用 |
| **JDK 23** | G1 / ZGC | 分代 ZGC 成为默认模式 (JEP 474) |
| **JDK 26+** | G1 / ZGC | 非分代 ZGC 已移除 (JEP 490)，分代 Shenandoah 实验性 (JEP 404) |
| **JDK 26+** | G1 / ZGC / Shenandoah | 分代 Shenandoah 生产就绪 (JEP 521) |
...
| 版本 | 特性 | JEP | 状态 |
|------|------|-----|------|
| JDK 11 | ZGC 引入 | JEP 333 | 实验性 |
| JDK 15 | ZGC 生产就绪 | JEP 377 | 正式版 |
| JDK 21 | 分代 ZGC 引入 | JEP 439 | 正式版 (需 `-XX:+ZGenerational` 启用) |
| JDK 23 | 分代 ZGC 成为默认 | JEP 474 | 默认模式，非分代模式已废弃 |
| JDK 24 | 移除非分代模式 | JEP 490 | 仅保留分代模式，`ZGenerational` 选项废弃 |
...
| JDK 15 | 生产就绪 | JEP 379 | 正式版 |
| JDK 26 | 分代 Shenandoah 引入 | JEP 404 | 实验性 (需 `-XX:+UnlockExperimentalVMOptions`) |
| JDK 26 | 分代 Shenandoah 生产就绪 | JEP 521 | 正式版 (不再需要实验性标志) |
...
**后续演进**:
- JDK 23 (JEP 474): 分代模式成为默认，非分代模式废弃
- JDK 24 (JEP 490): 非分代模式代码完全移除，`ZGenerational` 选项废弃

### JDK 25: 分代 Shenandoah (生产就绪)

**JEP 521: Generational Shenandoah (Product)**

JEP 521 将分代 Shenandoah 从实验性特性 (JEP 404, JDK 26) 提升为生产就绪的产品特性。经过 DaCapo、SPECjbb2015 等基准测试及多平台实际部署验证。

```bash
# JDK 25: 启用分代模式 (不再需要 UnlockExperimentalVMOptions)
-XX:+UseShenandoahGC
...
通过引入双卡表 (dual card table) 机制消除应用线程与 GC 优化器线程之间的同步开销。应用线程更新第一张卡表 (无需同步)，优化器线程处理第二张卡表，两者通过原子交换切换。

```bash
# 自动启用，无需配置
# x64 上写屏障从约 50 条指令减少到仅 12 条指令
# 额外内存开销: 约 0.2% 堆空间 (每 GB 堆约 2MB 原生内存)
```

**性能改进**:
- 重度对象引用修改场景: 吞吐量提升 5-15%
- 轻度引用修改场景: 仍有约 5% 吞吐量提升
- G1 吞吐量接近 Parallel GC 水平，同时保持低延迟和可预测停顿
- 无需配置更改
...
JEP 519 将紧凑对象头从实验性特性 (JEP 450, JDK 25) 提升为产品特性。已在 Oracle 全套 JDK 测试和 Amazon 数百个生产服务中验证。
...
| JDK 26 | G1 GC | 分代 Shenandoah 生产就绪 (JEP 521) |
...
│  ├── William Kemper: JEP 521 分代 Shenandoah (ex-Red Hat)       │
│  └── Amazon Corretto 维护                                       │
│                                                                 │
│  Datadog (5%+)                                                  │
│  ├── Roman Kennke: JEP 519 紧凑对象头 (ex-Red Hat→Amazon)       │
│  └── Zhengyu Gu: Shenandoah 核心开发者                           │
│                                                                 │
│  其他 (5%)                                                      │
│  ├── SAP: AArch64 优化                                          │
│  ├── IBM: PowerPC 支持                                          │
...
| **JEP 521** | Generational Shenandoah (Product) | William Kemper | Amazon (ex-Red Hat) | JDK 26 |
...
│  ├── 优化方向：分代收集 (JEP 439), NUMA 优化 (JDK 21)          │
│  └── 演进趋势：成为超低延迟场景首选                             │
│                                                                 │
...
| **2024** | JDK 24 持续优化 | JDK 26 分代默认 (JEP 474), JDK 24 移除非分代 (JEP 490) | JDK 24 分代实验 (JEP 404) |
| **2025** | JDK 26 持续优化 | JDK 25 持续优化 | JDK 25 分代生产就绪 (JEP 521) |
