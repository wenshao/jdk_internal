...
> **30 秒速读**
> - JDK 9 起成为默认 GC，Region 化堆布局，每次只回收垃圾最多的 Region 子集 (CSet)
> - Region 大小 1MB-32MB (自动计算: max_heap/2048)，目标 ~2048 个 Region
> - 默认目标暂停 `-XX:MaxGCPauseMillis=200ms`，通过自适应调整 CSet 大小实现
> - 支持 Humongous 对象 (≥Region 50%)、Region Pinning (JEP 423)
> - 并发标记 + 增量回收，Mixed GC 同时回收 Young + 部分 Old Region
> - 源码入口: `G1HeapRegion : public CHeapObj<mtGC>` (g1HeapRegion.hpp)
...
- [ZGC 深入](zgc.md) — 低延迟 GC，与 G1 形成互补选择
- [Shenandoah GC](shenandoah.md) — 另一款低暂停 GC，并发压缩策略不同
- [CMS 收集器](cms.md) — G1 的前身，了解演进脉络
