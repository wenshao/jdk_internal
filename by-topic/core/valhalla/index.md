...
2. [TL;DR](#2-tldr)
3. [项目概述](#3-项目概述)
4. [值类型 (Value Types)](#4-值类型-value-types)
5. [增强泛型](#5-增强泛型)
6. [L-World](#6-l-world)
7. [时间线](#7-时间线)
8. [当前状态 (2025-2026)](#8-当前状态-2025-2026)
9. [JEP 进展](#9-jep-进展)
10. [核心贡献者](#10-核心贡献者)
11. [示例对比](#11-示例对比)
12. [当前最佳实践](#12-当前最佳实践)
...
| **2026** | JDK 26 JEP 401 (Preview) | Value Classes and Objects 首次预览 |
| **2026** | JDK 26 Primitive Patterns (4th Preview) | JEP 530 第四预览 |
| **2027+** | 逐步交付 | Value Classes 正式版、Unified Generics |
...
**JEP 401 Value Classes 语法** (需 `--enable-preview`):
```java
// 使用 value 修饰符声明值类
value class Point {
    int x;  // 字段隐式 final
...
// 值对象无标识，== 比较字段值
// JVM 可将值对象扁平化到数组和字段中
// LocalDate 等 JDK 类在预览模式下已改为 value class，数组操作提速近 3x
```

**堆扁平化与标量化**:
- **堆扁平化** (Heap Flattening): 值对象直接嵌入到容器字段和数组中，减少内存占用
- **标量化** (Scalarization): JIT 编译代码中避免值对象的堆分配

### 开发中特性

| 特性 | JEP | 目标版本 | 状态 |
|------|-----|----------|------|
| **Value Classes** | JEP 401 | JDK 26 (Preview) | 🆕 首次预览 |
...
| 贡献者 | 组织 | 主要贡献 |
|--------|------|----------|
| Tobias Hartmann | Oracle | C2 JIT 编译器 |
...
| Vicente Romero | Oracle | javac 编译器 |
| [David Beaumont](/by-contributor/profiles/david-beaumont.md) | Oracle | javac、构建系统 |
| Gavin Bierman | Oracle | 语言规范 |
| Maurizio Cimadamore | Oracle | 类型系统 |
...
| Frederic Parain | Oracle | 字段、JNI |
| Daniel D. Daugherty | Oracle | 测试基础设施 |
| Markus Grönlund | Oracle | JFR |
...
| 贡献者 | 组织 | 主要贡献 |
|--------|------|----------|
| John Rose | Oracle | 项目创始人 |
| Aleks Seovic | - | 早期值类型原型 |
...
// Valhalla 设计 (JDK 26+ --enable-preview)
