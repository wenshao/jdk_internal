...
```
JDK 1.0 ─── JDK 8 ─── JDK 11 ─── JDK 17 ─── JDK 21 ─── JDK 24 ─── JDK 25 ─── JDK 26
   │           │            │            │            │           │           │           │
 JSSE       TLS 1.2      TLS 1.3      KMAC        后量子签名  ML-KEM     KDF API    ML-DSA JAR
 HTTPS                   ChaCha20     SHA-3       KDF API    ML-DSA     (正式)      HPKE
                        (JEP 329,332)             (JEP 495)  KDF 预览   JEP 510    JEP 518
                                                             JEP 496,497           JEP 517
```
...
| **JDK 21** | 签名增强 | HSS/LMS 签名、KEM API |
| **JDK 24** | 后量子基础 | ML-KEM (JEP 496, FIPS 203)、ML-DSA (JEP 497, FIPS 204)、KDF API 预览 (JEP 478) |
| **JDK 25** | KDF 正式化 | KDF API 正式版 (JEP 510) |
| **JDK 26** | 后量子应用 | ML-DSA JAR 签名 (JEP 518)、HPKE 混合加密、HTTP/3 预览 (JEP 517) |
...
| JDK 24 | **ML-KEM** 量子抗性密钥封装 (FIPS 203)、**ML-DSA** 量子抗性签名 (FIPS 204)、KDF API 预览 | JEP 496, JEP 497, JEP 478 |
| JDK 25 | **KDF API** (正式)、安全提供者增强 | JEP 510 |
| JDK 26 | **ML-DSA JAR 签名**、**HPKE 混合公钥加密**、**HTTP/3** (预览) | JEP 518, JEP 517 |
...
JDK 的后量子密码学 (PQC) 采用分阶段方式推进，与 NIST 标准化进程同步：

| 阶段 | 版本 | 内容 | NIST 标准 |
|------|------|------|-----------|
| 算法引入 | JDK 24 | ML-KEM (密钥封装)、ML-DSA (数字签名) | FIPS 203, FIPS 204 |
| 密钥派生 | JDK 25 | KDF API 正式化，支撑 ML-KEM 密钥派生 | - |
| 签名应用 | JDK 26 | ML-DSA 用于 JAR 签名、HPKE 混合加密 | FIPS 204 |
| 混合 TLS | JDK 27 (计划) | 后量子混合密钥交换 (JEP 527): X25519MLKEM768、SecP256r1MLKEM768 | - |

> **ML-KEM 参数集**: ML-KEM-512、ML-KEM-768、ML-KEM-1024 (安全级别递增)
> **ML-DSA 参数集**: ML-DSA-44、ML-DSA-65、ML-DSA-87 (安全级别递增)
...
| 后量子算法 | ML-KEM (FIPS 203), ML-DSA (FIPS 204) |
| 密码算法 | 100+ |
| Locale 数量 | 150+ |
