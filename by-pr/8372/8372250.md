# JDK-8372250: Merge PtrQueue into SATBMarkQueue 

## 基本信息

| 属性 | 值 |
|------|-----|
| **Issue** | [JDK-8372250](https://bugs.openjdk.org/browse/JDK-8372250) |
| **PR** | [#28755](https://github.com/openjdk/jdk/pull/28755) |
| **Author** | benty-amzn |
| **Created** | 2025-12-10 |
| **Merged** | 2025-12-12 |
| **审查天数** | 2 |
| **版本** | JDK 26 |
| **优先级** | P4 |
| **组件** | hotspot |
| **模块** | hotspot |
| **代码变更** | +188 / -302 (6 files) |
| **审查者** | earthling-amzn, kimbarrett, tschatzl, walulyai (4 人) |

## 描述

This change merges `PtrQueue` into its only child class, `SATBMarkQueue`. The corresponding `PtrQueueSet` is merged with `SATBMarkQueueSet`, and `G1SATBMarkQueueSet` and `ShenandoahSATBMarkQueueSet` continue to inherit from that class.

There are few code modifications - most methods were moved directly from `PtrQueue` to `SATBMarkQueue`. The one meaningful change is to  preserve `SATBMarkQueue`'s call to `filter(queue);` before `flush_queue`. 

This change passes all tier1 jtreg tests with both Shenandoah and G1 in local testing.

<!-- Anything below this marker will be automatically updated, please do not edit manually! -->
---------
### Progress
- [x] Change must be properly reviewed (1 review required, with at least 1 [Reviewer](https://openjdk.org/bylaws#reviewer))
- [x] Change must not contain extraneous whitespace
- [x] Commit message must refer to an issue

### Issue
 * [JDK-8372250](https://bugs.openjdk.org/browse/JDK-8372250): Merge PtrQueue into SATBMarkQueue (**Enhancement** - P4)


### Reviewers
 * [Kim Barrett](https://openjdk.org/census#kbarrett) (@kimbarrett - **Reviewer**)
 * [Ivan Walulya](https://openjdk.org/census#iwalulya) (@walulyai - **Reviewer**)
 * [Thomas Schatzl](https://openjdk.org/census#tschatzl) (@tschatzl - **Reviewer**)
 * [William Kemper](https://openjdk.org/census#wkemper) (@earthling-amzn - **Reviewer**)

### Reviewing
<details><summary>Using <code>git</code></summary>

Checkout this PR locally: \
`$ git fetch https://git.openjdk.org/jdk.git pull/28755/head:pull/28755` \
`$ git checkout pull/28755`

Update a local copy of the PR: \
`$ git checkout pull/28755` \
`$ git pull https://git.openjdk.org/jdk.git pull/28755/head`

</details>
<details><summary>Using Skara CLI tools</summary>

Checkout this PR locally: \
`$ git pr checkout 28755`

View PR using the GUI difftool: \
`$ git pr show -t 28755`

</details>
<details><summary>Using diff file</summary>

Download this PR as a diff file: \
<a href="https://git.openjdk.org/jdk/pull/

---

> **生成时间**: 2026-03-25
