# JDK-8314495: Update to use jtreg 7.3.1

## 基本信息

| 属性 | 值 |
|------|-----|
| **Issue** | [JDK-8314495](https://bugs.openjdk.org/browse/JDK-8314495) |
| **PR** | [#15323](https://github.com/openjdk/jdk/pull/15323) |
| **Author** | sormuras |
| **组织** | Oracle |
| **Created** | 2023-08-17 |
| **Merged** | 2023-08-21 |
| **审查天数** | 4 |
| **版本** | JDK 21 |
| **优先级** | P3 |
| **组件** | infrastructure |
| **模块** | test |
| **代码变更** | +9 / -12 (9 files) |
| **审查者** | dholmes-ora, erikj79, irisclark, jaikiran (4 人) |

## 描述

Please review the change to update to using jtreg 7.3,1.

The primary change is to the `jib-profiles.js` file, which specifies the version of jtreg to use, for those systems that rely on this file. In addition, the `requiredVersion` has been updated in the various `TEST.ROOT` files.

This change set also un-problem-lists tests from https://github.com/openjdk/jdk/commit/360f65d7b15b327e2f160c42f318945cc6548bda

This change set also fixes:
- https://bugs.openjdk.org/browse/JDK-8313902
- https://bugs.openjdk.org/browse/JDK-8313903

Testing: tier1-tier5 OK

<!-- Anything below this marker will be automatically updated, please do not edit manually! -->
---------
### Progress
- [x] Change must be properly reviewed (1 review required, with at least 1 [Reviewer](https://openjdk.org/bylaws#reviewer))
- [x] Change must not contain extraneous whitespace
- [x] Commit message must refer to an issue

### Issue
 * [JDK-8314495](https://bugs.openjdk.org/browse/JDK-8314495): Update to use jtreg 7.3.1 (**Enhancement** - P3)


### Reviewers
 * [David Holmes](https://openjdk.org/census#dholmes) (@dholmes-ora - **Reviewer**)
 * [Erik Joelsson](https://openjdk.org/census#erikj) (@erikj79 - **Reviewer**)
 * [Iris Clark](https://openjdk.org/census#iris) (@irisclark - **Reviewer**)
 * [Jaikiran Pai](https://openjdk.org/census#jpai) (@jaikiran - **Reviewer**)


### Reviewing
<details><summary>Using <code>git</code></summary>

Checkout this PR locally: \
`$ git fetch https://git.openjdk.org/jdk.git pull/15323/head:pull/15323` \
`$ git checkout pull/15323`

Update a local copy of the PR: \
`$ git checkout pull/15323` \
`$ git pull https://git.openjdk.org/jdk.git pull/15323/head`

</details>
<details><summary>Using Skara CLI tools</summary>

Checkout this PR locally: \
`$ git pr checkout 15323`

View PR using the GUI difftool: \
`$ git pr show -t 15323`

</details>
<details><summary>Using diff file</summary>

Download this PR as a diff file: \
<a href="https://git.openjdk.org/jdk/pull


> **Backported to**: JDK 11u, JDK 17u

---

> **生成时间**: 2026-03-25
