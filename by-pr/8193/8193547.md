# JDK-8193547: Regression automated test '/open/test/jdk/java/awt/Toolkit/DesktopProperties/rfe4758438.java' fails

## 基本信息

| 属性 | 值 |
|------|-----|
| **Issue** | [JDK-8193547](https://bugs.openjdk.org/browse/JDK-8193547) |
| **PR** | [#11127](https://github.com/openjdk/jdk/pull/11127) |
| **Author** | azvegint |
| **组织** | Oracle |
| **Created** | 2022-11-14 |
| **Merged** | 2022-12-14 |
| **审查天数** | 30 |
| **版本** | JDK 20 |
| **优先级** | P3 |
| **组件** | client-libs |
| **模块** | test |
| **代码变更** | +103 / -53 (3 files) |
| **审查者** | mrserb (1 人) |

## 描述

This test has multiple issues:

* Test actually does nothing and passes as it is right now, but tests nothing
* `pgrep gnome-session` may return multiple PIDs -> picking up only first one 
* looks like jtreg does not pass GNOME_DESKTOP_SESSION_ID variable(it is deprecated, BTW), so used `XDG_CURRENT_DESKTOP` instead
* removed leftovers from Solaris support
* `org.gnome.settings-daemon.peripherals.mouse` is [deprecated](https://gitlab.gnome.org/GNOME/gnome-settings-daemon/-/blob/master/data/org.gnome.settings-daemon.peripherals.gschema.xml.in#L46) and does not work on latest systems  <br> but newer `org.gnome.desktop.peripherals.mouse` works as expected<br>So the test modified to try new one and to check deprecated as a fallback

CI testing looks good.

<!-- Anything below this marker will be automatically updated, please do not edit manually! -->
---------
### Progress
- [x] Change must be properly reviewed (1 review required, with at least 1 [Reviewer](https://openjdk.org/bylaws#reviewer))
- [x] Change must not contain extraneous whitespace
- [x] Commit message must refer to an issue

### Issue
 * [JDK-8193547](https://bugs.openjdk.org/browse/JDK-8193547): Regression automated test '/open/test/jdk/java/awt/Toolkit/DesktopProperties/rfe4758438.java' fails


### Reviewers
 * [Sergey Bylokhov](https://openjdk.org/census#serb) (@mrserb - **Reviewer**) ⚠️ Review applies to [a16df68e](https://git.openjdk.org/jdk/pull/11127/files/a16df68edc5e1b171e37adab0277ec4eabb7a5cb)


### Reviewing
<details><summary>Using <code>git</code></summary>

Checkout this PR locally: \
`$ git fetch https://git.openjdk.org/jdk pull/11127/head:pull/11127` \
`$ git checkout pull/11127`

Update a local copy of the PR: \
`$ git checkout pull/11127` \
`$ git pull https://git.openjdk.org/jdk pull/11127/head`

</details>
<details><summary>Using Skara CLI tools</summary>

Checkout this PR locally: \
`$ git pr checkout 11127`

View PR using the GUI difftool: \
`$ git pr show -t 11127`

</detail


> **Backported to**: JDK 17u

---

> **生成时间**: 2026-03-25
