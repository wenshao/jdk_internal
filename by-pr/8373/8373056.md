# JDK-8373056: Shenandoah: Remove unnecessary use of ShenandoahAllocRequest.type()

## 基本信息

| 属性 | 值 |
|------|-----|
| **Issue** | [JDK-8373056](https://bugs.openjdk.org/browse/JDK-8373056) |
| **PR** | [#28649](https://github.com/openjdk/jdk/pull/28649) |
| **Author** | pengxiaolong |
| **组织** | Amazon |
| **Created** | 2025-12-03 |
| **Merged** | 2025-12-04 |
| **审查天数** | 1 |
| **版本** | JDK 26 |
| **优先级** | P4 |
| **组件** | hotspot |
| **模块** | gc/shenandoah |
| **代码变更** | +59 / -68 (6 files) |
| **审查者** | earthling-amzn, kdnilsen, pengxiaolong (3 人) |

## 描述

Follow up on the feedback/comments on PR https://github.com/openjdk/jdk/pull/28521 for bug [JDK-8372566](https://bugs.openjdk.org/browse/JDK-8372566), we should avoid using ShenandoahAllocRequest.type() directly if possible, in most of cases we don't need to directly use alloc type, the inline member methods provided by ShenandoahAllocRequest should be sufficient.

In the PR, I have removed most of the places where ShenandoahAllocRequest.type() directly used, there will be only one place left after the change:
* ShenandoahFreeSet::allocate (This one will be removed with PR https://github.com/openjdk/jdk/pull/26171)

Also did small code rearrangement for ShenandoahOldGeneration::configure_plab_for_current_thread 

### Test
- [x] hotspot_gc_shenandoah
- [x] GHA

<!-- Anything below this marker will be automatically updated, please do not edit manually! -->
---------
### Progress
- [x] Change must be properly reviewed (1 review required, with at least 1 [Reviewer](https://openjdk.org/bylaws#reviewer))
- [x] Change must not contain extraneous whitespace
- [x] Commit message must refer to an issue

### Issue
 * [JDK-8373056](https://bugs.openjdk.org/browse/JDK-8373056): Shenandoah: Remove unnecessary use of ShenandoahAllocRequest.type() (**Enhancement** - P4)


### Reviewers
 * [William Kemper](https://openjdk.org/census#wkemper) (@earthling-amzn - **Reviewer**)
 * [Kelvin Nilsen](https://openjdk.org/census#kdnilsen) (@kdnilsen - Committer)

### Reviewing
<details><summary>Using <code>git</code></summary>

Checkout this PR locally: \
`$ git fetch https://git.openjdk.org/jdk.git pull/28649/head:pull/28649` \
`$ git checkout pull/28649`

Update a local copy of the PR: \
`$ git checkout pull/28649` \
`$ git pull https://git.openjdk.org/jdk.git pull/28649/head`

</details>
<details><summary>Using Skara CLI tools</summary>

Checkout this PR locally: \
`$ git pr checkout 28649`

View PR using the GUI difftool: \
`$ git pr show -t 28649`

</details>
<details><summary>U

---

> **生成时间**: 2026-03-25
