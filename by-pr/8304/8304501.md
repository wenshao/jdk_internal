# JDK-8304501: Remove orphaned demo netbeans projects

## 基本信息

| 属性 | 值 |
|------|-----|
| **Issue** | [JDK-8304501](https://bugs.openjdk.org/browse/JDK-8304501) |
| **PR** | [#13098](https://github.com/openjdk/jdk/pull/13098) |
| **Author** | eirbjo |
| **组织** | Independent |
| **Created** | 2023-03-20 |
| **Merged** | 2023-04-11 |
| **审查天数** | 22 |
| **版本** | JDK 21 |
| **优先级** | P4 |
| **组件** | client-libs |
| **模块** | client |
| **代码变更** | +0 / -3063 (36 files) |
| **审查者** | AlanBateman, mrserb (2 人) |

## 描述

Please review this PR which suggests we remove the orhphaned `SwingApplet` netbeans project directory.

The `SwingApplet` demo was removed in [JDK-8205119](https://bugs.openjdk.org/browse/JDK-8205119), but the netbeans projects files were left behind.

Additionally, the following netbeans projects reference source files that do not exist (if they ever did?):

- src/demo/share/nbproject/management/FullThreadDump
- src/demo/share/nbproject/management/JTop
- src/demo/share/nbproject/management/MemoryMonitor
- src/demo/share/nbproject/management/VerboseGC
- src/demo/share/nbproject/scripting/jconsole-plugin

The PR deletes files in these directories as well.

<!-- Anything below this marker will be automatically updated, please do not edit manually! -->
---------
### Progress
- [x] Change must be properly reviewed (1 review required, with at least 1 [Reviewer](https://openjdk.org/bylaws#reviewer))
- [x] Change must not contain extraneous whitespace
- [x] Commit message must refer to an issue

### Issue
 * [JDK-8304501](https://bugs.openjdk.org/browse/JDK-8304501): Remove orphaned demo netbeans projects


### Reviewers
 * [Sergey Bylokhov](https://openjdk.org/census#serb) (@mrserb - **Reviewer**)


### Reviewing
<details><summary>Using <code>git</code></summary>

Checkout this PR locally: \
`$ git fetch https://git.openjdk.org/jdk.git pull/13098/head:pull/13098` \
`$ git checkout pull/13098`

Update a local copy of the PR: \
`$ git checkout pull/13098` \
`$ git pull https://git.openjdk.org/jdk.git pull/13098/head`

</details>
<details><summary>Using Skara CLI tools</summary>

Checkout this PR locally: \
`$ git pr checkout 13098`

View PR using the GUI difftool: \
`$ git pr show -t 13098`

</details>
<details><summary>Using diff file</summary>

Download this PR as a diff file: \
<a href="https://git.openjdk.org/jdk/pull/13098.diff">https://git.openjdk.org/jdk/pull/13098.diff</a>

</details>


### Webrev
[Link to Webrev Comment](https://git.openjdk.org/jdk/pull

---

> **生成时间**: 2026-03-25
