# JDK-8351146: JFR: JavaMonitorInflate event should default to no threshold and be disabled

## 基本信息

| 属性 | 值 |
|------|-----|
| **Issue** | [JDK-8351146](https://bugs.openjdk.org/browse/JDK-8351146) |
| **PR** | [#23891](https://github.com/openjdk/jdk/pull/23891) |
| **Author** | shipilev |
| **组织** | Amazon |
| **Created** | 2025-03-04 |
| **Merged** | 2025-03-11 |
| **审查天数** | 7 |
| **版本** | JDK 25 |
| **优先级** | P4 |
| **组件** | hotspot |
| **模块** | runtime/jfr |
| **代码变更** | +3 / -3 (2 files) |
| **审查者** | coleenp, egahlin (2 人) |

## 描述

There is a threshold for inflate event. But the operation that is covered by that event is very fast, and often lock-free, so the threshold would filter many events. This would be important as we add deflation event ([JDK-8351142](https://bugs.openjdk.org/browse/JDK-8351142)), which should match inflations.

The only way I see this event be longer than default threshold of 10ms is transiently waiting for another thread to complete inflation.

Additional testing:
 - [x] Linux x86_64 server fastdebug, `jdk_jfr` still passes

<!-- Anything below this marker will be automatically updated, please do not edit manually! -->
---------
### Progress
- [x] Change must be properly reviewed (1 review required, with at least 1 [Reviewer](https://openjdk.org/bylaws#reviewer))
- [x] Change must not contain extraneous whitespace
- [x] Commit message must refer to an issue

### Issue
 * [JDK-8351146](https://bugs.openjdk.org/browse/JDK-8351146): JFR: JavaMonitorInflate event should default to no threshold and be disabled (**Enhancement** - P4)


### Reviewers
 * [Coleen Phillimore](https://openjdk.org/census#coleenp) (@coleenp - **Reviewer**)
 * [Erik Gahlin](https://openjdk.org/census#egahlin) (@egahlin - **Reviewer**)

### Reviewing
<details><summary>Using <code>git</code></summary>

Checkout this PR locally: \
`$ git fetch https://git.openjdk.org/jdk.git pull/23891/head:pull/23891` \
`$ git checkout pull/23891`

Update a local copy of the PR: \
`$ git checkout pull/23891` \
`$ git pull https://git.openjdk.org/jdk.git pull/23891/head`

</details>
<details><summary>Using Skara CLI tools</summary>

Checkout this PR locally: \
`$ git pr checkout 23891`

View PR using the GUI difftool: \
`$ git pr show -t 23891`

</details>
<details><summary>Using diff file</summary>

Download this PR as a diff file: \
<a href="https://git.openjdk.org/jdk/pull/23891.diff">https://git.openjdk.org/jdk/pull/23891.diff</a>

</details>
<details><summary>Using Webrev</summary>

[Link to Webrev Comment](

---

> **生成时间**: 2026-03-25
