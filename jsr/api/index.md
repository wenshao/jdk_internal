# API 规范索引

> Java API 规范相关 JSR

---

## 概览

| JSR | 标题 | JDK | 状态 | 说明 |
|-----|------|-----|------|------|
| [JSR 166](https://jcp.org/en/jsr/detail?id=166) | Concurrency Utilities | 5 | ✅ Final | 并发工具 |
| [JSR 203](https://jcp.org/en/jsr/detail?id=203) | NIO.2 | 7 | ✅ Final | 新 I/O API |
| [JSR 310](jsr-310.md) ⭐ | Date and Time API | 8 | ✅ Final | 日期时间 API |
| [JSR 353](https://jcp.org/en/jsr/detail?id=353) | JSON Processing | 8 | ✅ Final | JSON 处理 |
| [JSR 366](https://jcp.org/en/jsr/detail?id=366) | HTTP Client | 11 | ✅ Final | HTTP 客户端 |

> ⭐ = 有本地详细文档

---

## 重要 JSR

- [JSR 310: Date and Time API](jsr-310.md) ⭐ - 本地详细文档

### 外部链接

- [JSR 166: Concurrency Utilities](https://jcp.org/en/jsr/detail?id=166)
- [JSR 203: NIO.2](https://jcp.org/en/jsr/detail?id=203)
- [JSR 353: JSON Processing](https://jcp.org/en/jsr/detail?id=353)
- [JSR 366: HTTP Client](https://jcp.org/en/jsr/detail?id=366)

---

## 相关链接

- [JCP JSR 列表](https://jcp.org/en/jsr/all)
- [JEP 索引](/jeps/)
